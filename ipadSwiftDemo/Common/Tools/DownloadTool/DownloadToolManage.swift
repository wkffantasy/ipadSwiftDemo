...
class DownloadToolManage: NSObject, URLSessionDelegate {
    ...
    // 开始下载url 和文件的总大小
    typealias DownloadStart = (String, String) -> Void
    // 下载失败
    ...
    func urlSession(_ session: URLSession, didReceive challenge: URLAuthenticationChallenge, completionHandler _: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
